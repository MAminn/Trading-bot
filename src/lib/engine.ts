...
import type { RequestedExecutionMode } from "./live-controls";
...
export interface EngineConfigRow {
  user_id: string;
  mode: EngineMode;
  /** Percentage of the REAL Binance wallet balance committed as margin.
   *  Independent of leverage. */
  capital_allocation_pct: number;
  /** Independent of capital_allocation_pct. */
  leverage: number;
  /** LEGACY. Retained for P&L-scaling displays and DB compatibility only.
   *  NOT an input to live order sizing — the capital base is the user's real
   *  Binance totalWalletBalance, read by the executor at order time. */
  capital_usd: number;
  ...
  // Generated by the database from `mode`; never written directly.
    ...
    mutationFn: (
      patch: Partial<
        Pick<
          ...
          | "demo_mode"
          | "execution_mode"
          // auto_execute_enabled is intentionally absent: it is a generated
          // column, written only by changing `mode`.
        >
      >,
    ) => fn({ data: patch }),
